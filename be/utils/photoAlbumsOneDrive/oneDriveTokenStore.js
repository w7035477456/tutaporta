...
import { decryptDriveRefreshToken, encryptDriveRefreshToken } from '../photoAlbumsDrive/driveTokenCrypto.js';
...
export const ONEDRIVE_CONN_KEY_PREFIX = 'v1:photo_albums:onedrive:conn:';
...
function connectionFromEncRow(row) {
  ...
  try {
    ...
    console.error('[photoAlbumsOneDrive] decrypt token failed:', err?.message || err);
    return null;
  }
...
async function readOneDriveConnectionFromDb(singlesId) {
  ...
  const row = rows[0];
  const enc = String(row?.record_photoalbums_onedrive_refresh_token_enc || '').trim();
  if (!enc) return null;
  return {
    enc,
    folderId: String(row.record_photoalbums_onedrive_folder_id || '').trim() || null,
    email: String(row.record_photoalbums_onedrive_email || '').trim() || null
  };
}
