/**
 * Gemini TTS bake for All Singles (/allSingles) page-instruction audio.
 * Writes fe/src/assets/sound/all_singles_instruction_{Persona}.m4a
 *
 ...
 *
 * Usage (from be/):
 *   node scripts/generateAllSinglesInstructionTutorialGemini.js
 *   node scripts/generateAllSinglesInstructionTutorialGemini.js --voice=Sora
 *   node scripts/generateAllSinglesInstructionTutorialGemini.js --all-voices
 *   node scripts/generateAllSinglesInstructionTutorialGemini.js --force
 */
import fs from 'fs';
...
/** Keep in sync with fe constants/allSinglesInstructionText.js */
const COPY = {
  title: 'Current Context Tutorial',
  step: "You are in 'All Singles' page.",
  body: [
    'Welcome to the very beginning of your beautiful new story! This is your space to dream big and explore. Want to narrow things down? Just tap the Edit Filter button to find exactly what your heart is looking for.',
    "See someone in the 'All Singles' gallery who makes your heart skip a beat? Simply tap My Pick to let the magic begin! From there, you can flutter over to Picks & Posts to wander through their world, read their stories, and see what makes them special.",
    "Also, here, we bring people together for more than just romance—we also help many find connections centered around shared hobbies and interests. Whether you're looking for romance, or searching for a buddy of same hobbies: discuss software hackathon, train for a marathon, or just hang out, you belong here.",
    "Browse profiles and tap 'My Picks' to bookmark anyone who catches your eye, or match your hobbies, so you can explore further. In your bio or intro video, please share whether you're looking for romance, hobby friends, or both!",
    "Go to 'Picks & Posts' page and you get further instructions."
  ].join(' ')
};

export const ALL_SINGLES_INSTRUCTION_GEMINI_VOICES = ['Sora', 'Jessica', 'Michael'];
export const ALL_SINGLES_INSTRUCTION_GEMINI_VOICE = 'Sora';
...
function toPersona(voice) {
  ...
  if (ALL_SINGLES_INSTRUCTION_GEMINI_VOICES.includes(name)) return name;
  return ALL_SINGLES_INSTRUCTION_GEMINI_VOICE;
}
...
function parseArgs(argv) {
  ...
  let allVoices = false;
  let force = false;
  const known = [...ALL_SINGLES_INSTRUCTION_GEMINI_VOICES, 'Sulafat', 'Achernar', 'Enceladus'];
  ...
  return {
    voices: allVoices || !voices.length ? [...ALL_SINGLES_INSTRUCTION_GEMINI_VOICES] : voices,
    force
  };
}
...
async function main() {
  ...
  for (const voice of voices) {
    ...
    const m4aPath = path.join(OUT_DIR, `all_singles_instruction_${persona}.m4a`);
    ...
    const wavPath = path.join(OUT_DIR, `all_singles_instruction_${persona}.wav`);
