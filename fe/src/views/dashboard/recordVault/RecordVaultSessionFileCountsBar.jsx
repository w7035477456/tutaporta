...
import { fetchRecordVaultSessionFileCounts } from 'api/recordVaultFe';
 ...
 */
export default function RecordVaultSessionFileCountsBar({ refreshToken = 0 }) {
  const [usbTxRx, setUsbTxRx] = useState(0);
  const [uiTxRx, setUiTxRx] = useState(0);

  const load = useCallback(async () => {
    try {
      const counts = await fetchRecordVaultSessionFileCounts();
...
RecordVaultSessionFileCountsBar.propTypes = {
