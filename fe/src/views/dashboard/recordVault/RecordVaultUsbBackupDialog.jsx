...
export default function RecordVaultUsbBackupDialog({
  ...
  const handleFormat = async () => {
    ...
    const ok = await themedConfirm(
      `Format TutaNotes USB on ${label}?\n\nThis deletes the existing USB TutaNotes folder and creates a fresh vault with SAMPLE NOTEBOOK (SAMPLE NOTE1 / SAMPLE NOTE2). Other files on the USB are not touched.\n\nBack up first if you want to keep your current notes.`
    );
    if (!ok) return;
    ...
    try {
      ...
      needsRelockRef.current = true;
      setVaultNeedsRelock(true);
      setSuccess(`Formatted TutaNotes USB on ${resultLabel}. Open TutaNotes again to see SAMPLE NOTEBOOK.`);
      setSuccessTone('general');
