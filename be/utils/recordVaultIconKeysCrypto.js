...
import {
  fingerprintRecordVaultMasterKeyRaw,
  resolveRecordVaultMasterKey
} from './recordVaultMasterKey.js';
...
function resolveKey(options = {}) {
  return resolveRecordVaultMasterKey(options.masterKeyRaw);
}

/** Safe fingerprint of an encrypted icon-keys blob (no plaintext). */
export function fingerprintRecordVaultIconKeysEncBlob(encBase64) {
...
export function encryptRecordVaultIconKeyMap(map, options = {}) {
...
export function decryptRecordVaultIconKeyMap(encBase64, options = {}) {
  const blob = String(encBase64 ?? '').trim();
  if (!blob) {
    throw new Error('Record Vault icon keys are not seeded (encrypted blob is empty)');
  }
  const blobFp = fingerprintRecordVaultIconKeysEncBlob(encBase64);
  ...
  const key = resolveKey(options);
  const keyFp = fingerprintRecordVaultMasterKeyRaw(
    options.masterKeyRaw != null
      ? options.masterKeyRaw
      : process.env.RECORD_NOTES_ICON_KEYS_MASTER_KEY
  );
