...
export const ONEDRIVE_EMAILS_KEY_PREFIX = 'v1:photo_albums:onedrive:emails:';
...
async function readOneDriveEmailsForPickerFromDb(singlesId) {
  ...
  const connected = normalizeEmail(row.record_photoalbums_onedrive_email);
  if (!connected) return saved;
  if (saved.some((entry) => entry.toLowerCase() === connected)) return saved;
  return [String(row.record_photoalbums_onedrive_email).trim(), ...saved];
}
