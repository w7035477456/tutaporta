...
async function loadVaultAccessHash(singlesId) {
  ...
  return String(row?.photoalbums_access_password_hash ?? '').trim();
}
...
export async function setVaultAccessPasswordEnabled(singlesId, enabled, options = {}) {
  ...
  if (!nextEnabled) {
    const row = await loadVaultAccessRow(id);
    const hash = String(row?.photoalbums_access_password_hash ?? '').trim();
    const currentlyEnabled = Boolean(row?.photoalbums_access_password_enabled);
...
export async function requireVaultAccessSession(req, res) {
  ...
  if (!(await isVaultAccessUnlocked(singlesId))) {
    res.status(428).json({
      ...
      code: 'PHOTO_ALBUMS_ACCESS_REQUIRED'
    });
    return null;
  }
  return singlesId;
