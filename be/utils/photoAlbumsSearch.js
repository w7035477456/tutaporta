...
function stripPhotoAlbumsHtmlForSearch(text) {
  return String(text ?? '').replace(/<[^>]+>/g, ' ');
}

/**
 * Lowercase / collapse whitespace for full-note search haystacks.
 * Unlike normalizePhotoAlbumsKeyword, this must NOT truncate — otherwise body
 ...
 */
function normalizePhotoAlbumsSearchHaystack(raw) {
  return String(raw ?? '')
    .trim()
    .toLowerCase()
    .replace(/\s+/g, ' ');
...
export function expandPhotoAlbumsBodyTextForSearch(bodyText) {
  ...
  if (raw.startsWith(NOTE_BODY_V2_PREFIX)) {
    try {
      ...
      return [parsed.before, parsed.afterTop, parsed.afterCenter, parsed.afterBottom]
        .map((part) => stripPhotoAlbumsHtmlForSearch(part).trim())
        .filter(Boolean)
        .join(' ');
    } catch {
  ...
  return stripPhotoAlbumsHtmlForSearch(raw);
}

/** Evaluate left-to-right: term1 (op1 term2) (op2 term3). Empty terms are skipped. */
export function evaluatePhotoAlbumsSearchChain(chain, noteMatchesTerm) {
...
export function noteMatchesPhotoAlbumsSearchTerm(note, notebookName, term) {
  const normalized = normalizePhotoAlbumsKeyword(term);
  if (!normalized) return true;
  const keywords = Array.isArray(note?.keywords) ? note.keywords : [];
  if (keywords.some((kw) => normalizePhotoAlbumsKeyword(kw) === normalized)) return true;
  if (keywords.some((kw) => normalizePhotoAlbumsKeyword(kw).includes(normalized))) return true;
  ...
  const hay = normalizePhotoAlbumsSearchHaystack(
    [
      stripPhotoAlbumsHtmlForSearch(note?.note_name),
      expandPhotoAlbumsBodyTextForSearch(note?.body_text),
      ...keywords
    ]
      .filter(Boolean)
      .join(' ')
...
export function buildPhotoAlbumsSearchChain(q1, q2, q3, op1, op2) {
...
export function normalizePhotoAlbumsKeyword(raw) {
  return String(raw ?? '')
    .trim()
    .toLowerCase()
    .replace(/\s+/g, ' ')
