...
import { PHOTO_ALBUMS_BRIDGE_PORT } from 'api/photoAlbumsBridgeFe';
import { downloadUsbBridgeInstaller, readPhotoAlbumsApiError } from 'api/photoAlbumsFe';
...
/**
 * TutaPhotoAlbums USB Login bridge strip — four states:
 ...
 */
export default function PhotoAlbumsUsbBridgeStatusPanel({
  ...
  const handleInstallerDownload = (event) => {
    ...
    void downloadUsbBridgeInstaller(downloadPlatform).catch((err) => {
      void themedAlert(readPhotoAlbumsApiError(err, 'USB Bridge installer download failed'));
    });
  };
  ...
  const listenLine = `Listening on 127.0.0.1:${PHOTO_ALBUMS_BRIDGE_PORT || 49201}`;
  ...
  if (status === 'ready') {
    ...
    // Task1 — bridge not reachable: point at always-visible download button.
    statusBody = (
      <Stack spacing={1} sx={{ width: '100%' }}>
  ...
  return (
    <Stack spacing={1} sx={{ width: '100%' }} {...guestDemoAllowProps()}>
      <GreenButton
        type="button"
        ...
        title={
          preferMac
            ? 'Download usbBridgeV3.dmg (from server FAST_STORAGE_FOLDER / USB_DMG_EXE)'
            : 'Download usbBridgeV3.exe (from server FAST_STORAGE_FOLDER / USB_DMG_EXE)'
        }
...
PhotoAlbumsUsbBridgeStatusPanel.propTypes = {
