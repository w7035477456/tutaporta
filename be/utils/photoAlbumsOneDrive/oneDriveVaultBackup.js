...
import { trackVaultTransferBytes } from '../photoAlbumsTransferTracking.js';
...
export function buildMyPhotoAlbumsBackupZipFileName(kind = 'onedrive') {
  const prefix = kind === 'usb' ? 'MyPhotoAlbums_USB' : 'MyPhotoAlbums_OneDrive';
  return `${prefix}_${formatMyPhotoAlbumsBackupZipStamp()}.zip`;
}

function backupZipFileName() {
  return buildMyPhotoAlbumsBackupZipFileName('onedrive');
}
