...
import TutaNotesRadioMark, {
  tutaNotesNativeRadioInputSx,
  tutaNotesRadioControlWrapSx
...
import { TUTANOTES_CLOUD_LOGO, TUTANOTES_CLOUD_LOGIN_TITLE } from './tutaNotesBranding';
...
export default function RecordVaultOneDriveLoginModal({
  open,
  embedded = false,
  defaultEmail = '',
  savedEmails = [],
  ...
  /** Valid TutaNotes cloud vault only — Open/View/Backup. Missing/broken → Format only. */
  ...
  const canViewOrBackupCloud = canOpenCloud;
    ...
    <>
      <ColorTemplate16PopupCenterWide.Title sx={oneDriveTitleSx}>
        <TutaNotesBrandTitle
          ...
          title={TUTANOTES_CLOUD_LOGIN_TITLE}
          logoSize={64}
          fitWidth
          ...
          <Stack spacing={1.5} sx={oneDriveFormStackSx}>
            <Box sx={controlsColumnSx}>
                    ...
                    >
                      {emailOptions.map((option) => {
                        ...
                        return (
                          ...
                          >
                            <Box sx={tutaNotesRadioControlWrapSx}>
                              <Box
                                component="input"
                                type="radio"
                                ...
                                sx={tutaNotesNativeRadioInputSx}
                              />
                              <TutaNotesRadioMark selected={checked} disabled={busy} />
                            </Box>
              ...
              <Box sx={oneDriveActionButtonStackSx}>
                <GreenButton
                  type="button"
                  disabled={!loginEmail || busy || sessionActive}
                  aria-busy={busy}
                  onClick={handleSubmit}
                >
                  {busy ? 'Connecting…' : 'Login OneDrive for TutaNotes'}
                </GreenButton>
              ...
              {sessionActive && (!hasVault || needsReformat) ? (
                ...
                >
                  {needsReformat
                    ? 'Cloud vault is invalid or outdated. Use Format TutaNotes Cloud, then set up again.'
                    : 'No valid TutaNotes vault on this OneDrive. Use Format TutaNotes Cloud to create one.'}
                </ColorTemplate16PopupCenterWide.SectionDescription>
              ) : null}
