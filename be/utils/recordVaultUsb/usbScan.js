...
/** Bridge installer DMG / app volume names — not usable as TutaNotes USB targets. */
function isBridgeInstallerVolumeLabel(label) {
  const name = String(label || '').trim().toLowerCase();
  if (!name) return false;
  return (
    /^record vault usb bridge(\s+\d+)?$/.test(name) ||
    /^tutanotes bridge installer(\s+\d+)?$/.test(name) ||
    /^omnotes bridge installer(\s+\d+)?$/.test(name)
  );
}
...
function vaultStatusForPath(mountPath) {
  ...
  // TutaNotes folder present but missing meta/db/photos/files → not a valid vault.
...
export function scanForRecordVaultUsb() {
