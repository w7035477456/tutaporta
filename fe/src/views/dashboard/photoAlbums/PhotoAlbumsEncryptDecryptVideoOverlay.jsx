...
/**
 * Shows Lock.gif centered over the current TutaPhotoAlbums workspace while inner
 * encrypt / decrypt runs. Keeps the same screen — no myPhotoAlbumsBackground splash.
 *
 * Stays visible until BOTH crypto finishes and one full GIF cycle completes.
 */
export default function PhotoAlbumsEncryptDecryptVideoOverlay({
  open = false,
  kind = 'encrypt',
  label,
  ...
  return createPortal(
    <Box
      sx={{
        ...
        // Block clicks while crypto runs, but keep TutaPhotoAlbums UI visible underneath.
        pointerEvents: 'all',
        cursor: 'wait',
        bgcolor: 'transparent',
        ...(sx || null)
...
PhotoAlbumsEncryptDecryptVideoOverlay.propTypes = {
  open: PropTypes.bool,
