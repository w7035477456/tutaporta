...
import { formatRecordVaultUsbAssignmentLabel } from 'utils/recordVaultUsbStatsLabel';
import { MAIN_FONT_FAMILY } from 'config/mainFontEnv';

export const RECORD_VAULT_USB_DRAG_MIME = 'application/x-record-vault-usb';

export function parseRecordVaultUsbDragPayload(event) {
  try {
    const raw = event.dataTransfer?.getData(RECORD_VAULT_USB_DRAG_MIME);
    if (!raw) return null;
...
export default function RecordVaultUsbAssignmentDropZone({
  label,
  location,
  dragActive = false,
  busy = false,
  ...
  const handleDrop = (event) => {
    event.preventDefault();
    const payload = parseRecordVaultUsbDragPayload(event);
    ...
    >
      ...
      {location ? (
        <SliderControlButton type="button" disabled sx={drivePillButtonSx}>
          {formatRecordVaultUsbAssignmentLabel(location)}
        </SliderControlButton>
      ) : (
