...
const LEGACY_ONEDRIVE_NOTES_FOLDER_NAMES = ['OMNotes', '.recordvault', 'onlinemallwebsitevault'];

/** OneDrive root folder for TutaNotes (default: TutaNotes). Override: ONEDRIVE_NOTES_FOLDER_NAME */
export function getOneDriveVaultFolderName() {
  const name = String(process.env.ONEDRIVE_NOTES_FOLDER_NAME || 'TutaNotes').trim();
  return name || 'TutaNotes';
}
...
export function isOneDriveVaultRootFolderName(name) {
  ...
  return LEGACY_ONEDRIVE_NOTES_FOLDER_NAMES.includes(folderName);
}

export function listOneDriveVaultRootFolderNamesToPurge() {
  return [...new Set([getOneDriveVaultFolderName(), ...LEGACY_ONEDRIVE_NOTES_FOLDER_NAMES])];
}
...
/** OAuth redirect for the FE tab that started login (localhost dev vs production). */
