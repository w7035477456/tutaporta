...
export function validateMediaStorage() {
  ...
  if (!photoDir) {
    issues.push('TUTADATES_PHOTO_FOLDER is not set');
  } else if (!fs.existsSync(photoDir)) {
    issues.push(`TUTADATES_PHOTO_FOLDER does not exist: ${photoDir}`);
  } else {
    try {
      fs.accessSync(photoDir, fs.constants.R_OK | fs.constants.W_OK);
    } catch {
      issues.push(`TUTADATES_PHOTO_FOLDER is not readable/writable: ${photoDir}`);
    }
    if (isMultiServerClusterMode() && isLikelyLocalOnlyPath(photoDir)) {
      issues.push(
        `TUTADATES_PHOTO_FOLDER looks local-only (${photoDir}) — use shared storage for CLUSTER_MULTI_SERVER`
      );
    }
  }
  ...
  if (videoDir && videoDir !== photoDir) {
    if (!fs.existsSync(videoDir)) {
      issues.push(`TUTADATES_VIDEO_FOLDER does not exist: ${videoDir}`);
    } else if (isMultiServerClusterMode() && isLikelyLocalOnlyPath(videoDir)) {
      issues.push(
        `TUTADATES_VIDEO_FOLDER looks local-only (${videoDir}) — use shared storage for CLUSTER_MULTI_SERVER`
      );
    }
  }
