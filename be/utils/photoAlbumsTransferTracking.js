 ...
 * wall-time multiple (`PHOTOALBUMS_OVERLIMIT_THROTTLE_MULTIPLE` in ~/.ssh/be/.env).
 * Example: 5 → a 1s transfer becomes ~5s. Default 10.
 */
export const VAULT_TRANSFER_OVERAGE_THROTTLE_FACTOR = (() => {
  const n = Number(process.env.PHOTOALBUMS_OVERLIMIT_THROTTLE_MULTIPLE);
...
/** Purchase block shown in TutaPhotoAlbums usage bar ("add 10GB block"). */
...
/** UI copy when PHOTOALBUMS_OVERLIMIT_THROTTLE_MULTIPLE is stretching transfers (line 2 under upload status). */
...
const PENDING_BYTES_PREFIX = 'v1:photo_albums:transfer_pending_bytes:';
const BYTES_PER_MB = 1024 * 1024;
const FLUSH_PENDING_BYTES_THRESHOLD = BYTES_PER_MB;
...
/** Deduct whole MB from Postgres; leave sub-MB remainder in Redis pending. */
async function deductVaultTransferMb(singlesId, mbWhole) {
  const id = Number(singlesId);
  const mb = Math.floor(Number(mbWhole) || 0);
  ...
  try {
    ...
    console.error('[vault-transfer] mb deduct failed:', err?.message || err);
    return 0;
  }
}
...
export async function addVaultTransferBytes(singlesId, bytes) {
  ...
  const mbWhole = Math.floor(b / BYTES_PER_MB);
  const remainder = b % BYTES_PER_MB;
  if (remainder > 0) {
    await clusterRedisIncrBy(pendingBytesKey(id), remainder);
  }
  if (mbWhole <= 0) return getVaultTransferStats(id).then((s) => s.refillRemainMb);
  return deductVaultTransferMb(id, mbWhole);
}
 ...
 * Operations still proceed; callers throttle wall time by PHOTOALBUMS_OVERLIMIT_THROTTLE_MULTIPLE.
 */
 ...
 * PHOTOALBUMS_OVERLIMIT_THROTTLE_MULTIPLE × elapsed when quota is exhausted.
...
export async function trackVaultTransferBytes(singlesId, bytes, startedAtMs = null, opts = {}) {
  ...
  const pendingAfter = await clusterRedisIncrBy(pendingBytesKey(id), b);
  if (pendingAfter >= FLUSH_PENDING_BYTES_THRESHOLD) {
...
/** Client / USB-bridge reported bytes (same Redis pending queue as API meter). */
export async function reportPhotoAlbumsTransferBytes(singlesId, bytes, startedAtMs = null, opts = {}) {
  return trackVaultTransferBytes(singlesId, bytes, startedAtMs, opts);
}
...
export async function getVaultTransferStats(singlesId) {
  ...
  try {
    ...
    const usedMb = Number(rows[0]?.photoalbums_total_transfer_mb) || 0;
    ...
    if (Number.isFinite(id) && id >= 1) {
      pendingBytes = Math.max(0, await clusterRedisGetInt(pendingBytesKey(id)));
    }
    const remainBytes = mbToBytes(refillRemainMb) - pendingBytes;
    const refillRemainMbExact = remainBytes / BYTES_PER_MB;
    const leftMb = refillRemainMbExact;
    const usedBytes = mbToBytes(usedMb) + pendingBytes;
    const leftPct =
      initialDataMb > 0
    ...
    return {
      ...
      refillRemainMb,
      refillRemainMbExact,
      refillRemainBytes: remainBytes,
      pendingTransferBytes: pendingBytes,
      refillBoughtMb,
      refillBlockMb: VAULT_REFILL_BLOCK_MB,
      overageThrottled: remainBytes <= 0
    };
  } catch (err) {
