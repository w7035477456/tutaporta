...
import { decryptRecordVaultIconKeyMap } from './recordVaultIconKeysCrypto.js';
...
export function normalizeRecordVaultIconName(raw) {
  return String(raw ?? '')
    .trim()
    .toLowerCase()
    .replace(/^fa-/, '');
...
export function formatRecordVaultIconLabel(iconName) {
  const name = normalizeRecordVaultIconName(iconName);
  if (!name) return '';
  return name
    .split('-')
...
export function listRecordVaultIconCatalog() {
  const raw = fs.readFileSync(ICON_LIST_PATH, 'utf8');
  const parsed = JSON.parse(raw);
  const icons = Array.isArray(parsed?.icons) ? parsed.icons : [];
  return [...new Set(icons.map((name) => normalizeRecordVaultIconName(name)).filter(Boolean))]
    .sort()
    .map((name) => ({ name, label: formatRecordVaultIconLabel(name) }));
}
...
async function loadIconKeyMap(force = false) {
  ...
  const map = encBlob ? decryptRecordVaultIconKeyMap(encBlob) : {};
  cachedMap = map;
  cacheLoadedAt = now;
  return map;
}

export async function getRecordVaultIconSecret(iconName) {
  const normalized = normalizeRecordVaultIconName(iconName);
  if (!normalized) {
...
export async function getRecordVaultIconEncryptionKey(iconName, options = {}) {
  const secret = await getRecordVaultIconSecret(iconName);
  return deriveVaultKeyFromIconSecret(secret, options);
}

export async function getRecordVaultEnvEncryptionKey(storageType, options = {}) {
  return deriveVaultKeyFromIconSecret(getRecordVaultEnvSecret(storageType), options);
}
...
export async function createIconVaultKeyMaterial(iconName) {
  ...
  const key = await getRecordVaultIconEncryptionKey(iconName, {
...
export async function createEnvVaultKeyMaterial(storageType) {
  const salt = crypto.randomBytes(ICON_VAULT_KDF.saltLength);
  const key = await getRecordVaultEnvEncryptionKey(storageType, {
    kdf: ICON_VAULT_KDF_ARGON2ID,
    salt
  });
  return {
...
export async function listEnvVaultUnlockKeys(storageType, meta) {
  return listVaultUnlockKeysFromSecret(getRecordVaultEnvSecret(storageType), meta);
}
...
export function invalidateRecordVaultIconKeyCache() {
  cachedMap = null;
  cacheLoadedAt = 0;
}
