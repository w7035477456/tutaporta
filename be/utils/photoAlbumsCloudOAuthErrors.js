...
export function formatMicrosoftOAuthProviderError({
  ...
  const hintsByCode = {
    ...
    unauthorized_client: hintLines([
      ...
      'Azure → App registrations → Authentication → add Web redirect URI for localhost: http://localhost:3000/api/photoAlbums/onedrive/oauth/callback',
...
export function formatOAuthSaveError(provider, err, { singlesId, columnHint = '' } = {}) {
  ...
  const secondary = isMissingColumn
    ? hintLines([
        ...
        columnHint ||
          'Run be/db/addSinglesPhotoAlbumsCloud.sql on Postgres Primary, then pm2 restart onlinemallwebsite.'
      ])
    : hintLines([
        `Server error: ${message}.`,
        singlesId ? `singles_id=${singlesId}.` : '',
        'Check PM2 logs for [PhotoAlbumsCloud:OneDrive] connect oauth/callback failed.'
      ]);
