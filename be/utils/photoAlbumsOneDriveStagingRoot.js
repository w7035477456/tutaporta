 ...
 * Uses RECORD_PHOTOALBUMS_ONEDRIVE_STAGING_ROOT when usable; otherwise per-host tmpdir (Mac dev safe).
 */
export function resolveOneDriveStagingRoot() {
  if (cachedEffectiveRoot) return cachedEffectiveRoot;

  const configuredRaw = process.env.RECORD_PHOTOALBUMS_ONEDRIVE_STAGING_ROOT;
  ...
  if (configured) {
    const msg = `RECORD_PHOTOALBUMS_ONEDRIVE_STAGING_ROOT not usable on this host (${configured}) — using ${fallback}`;
...
export function getOneDriveStagingRootStatus() {
  const configured = expandConfiguredStagingRoot(process.env.RECORD_PHOTOALBUMS_ONEDRIVE_STAGING_ROOT);
