...
export default function RecordVaultEditorToolbar({ editor }) {
  ...
  return (
    <div className="rv-tb" role="toolbar" aria-label="Editor toolbar">
      ...
      <div className="rv-tb__group">
        <Btn title="Link" active={s.isLink} onClick={() => void promptLink()}>
          🔗
        </Btn>
        <Btn title="Image" onClick={() => void promptImage()}>
          🖼
        </Btn>
        <Btn title="YouTube" onClick={() => void promptYoutube()}>
          ▶
        </Btn>
        <Btn title="Inline math" onClick={() => void promptMath()}>
          √x
        </Btn>
...
RecordVaultEditorToolbar.propTypes = {
