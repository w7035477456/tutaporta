...
} from '../../utils/photoAlbumsOneDrive/oneDriveApi.js';
import { saveOneDriveConnection } from '../../utils/photoAlbumsOneDrive/oneDriveTokenStore.js';
import { formatEmptyOneDriveVaultFolderIfNeeded } from '../../utils/photoAlbumsOneDrive/oneDriveVaultSync.js';
import { rvCloudDebug, rvCloudError, rvCloudLog, rvCloudWarn } from '../../utils/photoAlbumsCloudDebugLog.js';
import {
  formatMicrosoftOAuthProviderError,
  formatOAuthSaveError,
  formatOAuthTokenExchangeError,
  sanitizeOAuthCallbackQuery
} from '../../utils/photoAlbumsCloudOAuthErrors.js';
import { isVaultOneDriveOffered } from '../../utils/photoAlbumsStorageFlags.js';

const OAUTH_RESULT_STORAGE_KEY = 'photoAlbumsOneDriveOAuthResult';
const OAUTH_BROADCAST_CHANNEL = 'photo-albums-onedrive-oauth';
const OAUTH_ACK_TYPE = 'photo-albums-onedrive-oauth-ack';
...
function createOneDriveOAuthState(returnOrigin, singlesId) {
  const payload = {
    ...
    s: Number(singlesId),
    // Azure redirect is shared with Notes; callback demuxes on this flag.
    p: 'photoAlbums'
  };
...
function verifyOneDriveOAuthState(stateRaw) {
  ...
  try {
    ...
    return { returnOrigin, singlesId, product: String(payload?.p || 'photoAlbums') };
  } catch {
    return null;
  }
}
...
function buildOAuthResultPayload({ success, email = '', error = '', errorSecondary = '', debug = null }) {
  return {
    type: 'photo-albums-onedrive-oauth',
...
/** GET /api/photoAlbums/onedrive/oauth/start */
export function photoAlbumsOneDriveOAuthStart(req, res) {
  ...
  if (!isVaultOneDriveOffered()) {
    rvCloudWarn('OneDrive', 'connect oauth/start rejected — not offered (PHOTOALBUMS_ONE_DRIVE=false)');
...
/** GET /api/photoAlbums/onedrive/oauth/callback */
export async function photoAlbumsOneDriveOAuthCallback(req, res) {
  const config = getMicrosoftOAuthConfig();
  const verifiedState = verifyOneDriveOAuthState(req.query?.state);
  ...
  try {
    ...
    try {
      ...
      const formatted = formatOAuthSaveError('OneDrive', saveErr, {
        singlesId: verifiedState.singlesId,
        columnHint: 'Run be/db/addSinglesPhotoAlbumsCloud.sql on Postgres Primary, then pm2 restart onlinemallwebsite.'
      });
      return res.status(200).send(
        renderPopupResultHtml({
          success: false,
    ...
    return res.status(200).send(
      renderPopupResultHtml({
        ...
        errorSecondary:
          'Check PM2 logs for [PhotoAlbumsCloud:OneDrive] connect oauth/callback. Verify Azure redirect URI, client secret, and Graph API permissions (User.Read, Files.ReadWrite).',
