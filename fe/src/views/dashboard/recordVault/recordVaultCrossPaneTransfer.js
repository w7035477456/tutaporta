...
async function tryImageDataUrl(noteId, slot, storageType) {
  try {
    const blob = await fetchRecordVaultNoteImageBlob(noteId, slot, { storageType });
...
async function tryExtraImageDataUrl(noteId, imageId, storageType) {
  try {
    const blob = await fetchRecordVaultNoteExtraImageBlob(noteId, imageId, { storageType });
    if (!blob || blob.size === 0) return null;
    return await blobToDataUrl(blob);
  } catch {
    return null;
...
export async function copyNoteBetweenVaults({
  ...
  const sourceApi = createRecordVaultPaneApi(sourceStorageType);
  const full = await sourceApi.fetchRecordVaultNote(sourceNoteId);
  ...
  const sourceHasInner =
    Boolean(full.inner_encrypt_enabled) || isRecordVaultInnerEncryptedBody(bodyText);
  ...
  const created = await targetApi.createRecordVaultNote(targetNotebookId, {
    note_name: noteName,
    body_text: bodyText,
    keywords,
    ...(copyInnerEncrypt
  ...
  if (Object.keys(patch).length) {
    await targetApi.updateRecordVaultNote(newNoteId, patch);
  }
  ...
  for (let i = 0; i < extras.length; i += 1) {
    ...
    if (!dataUrl) continue;
    await targetApi.uploadRecordVaultNoteExtraImage(newNoteId, { image: dataUrl });
  }
  ...
  for (let i = 0; i < attachments.length; i += 1) {
    ...
    if (!dataUrl) continue;
    await targetApi.uploadRecordVaultNoteAttachment(newNoteId, {
      file: dataUrl,
      file_name: fileName
    });
  }
 ...
 */
export async function transferRecordVaultItem({
  mode, // 'copy' | 'move'
  ...
  if (item.kind === 'notebook') {
    ...
    const sourceTree = await sourceApi.fetchRecordVaultTree();
    ...
    const createdNb = await targetApi.createRecordVaultNotebook(notebookName);
    ...
    if (mode === 'move') {
      ...
      await sourceApi.deleteRecordVaultNotebook(item.id);
    }
  ...
  if (mode === 'move') {
    ...
    await sourceApi.deleteRecordVaultNote(item.id);
  }
  await reportTransferProgress(onProgress, 100, 'Done');
