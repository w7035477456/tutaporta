 ...
 * Includes length, whitespace/control hints, and how resolveRecordVaultMasterKey would derive bytes.
 */
export function fingerprintRecordVaultMasterKeyRaw(raw) {
  ...
  let derivedFp = null;
  try {
    const derived = resolveRecordVaultMasterKey(value);
...
export function resolveRecordVaultMasterKey(raw) {
  const value =
    raw != null
      ? String(raw).trim()
      : String(process.env.RECORD_NOTES_ICON_KEYS_MASTER_KEY ?? '').trim();
  if (!value) {
    throw new Error('RECORD_NOTES_ICON_KEYS_MASTER_KEY is not configured');
  }
  try {
    const decoded = Buffer.from(value, 'base64url');
    if (decoded.length === KEY_LEN) {
