...
import { trackVaultTransferBytes, isVaultTransferQuotaExhausted, getVaultTransferThrottleRefillLabel } from '../vaultTransferTracking.js';
import {
  VAULT_FILES_DIR,
  VAULT_META_FILE,
  VAULT_PHOTOS_DIR,
...
import { getUnlockGuardStatusFromMeta } from '../recordVaultUsb/unlockGuard.js';
import { readVaultMeta, validateVaultOnMount } from '../recordVaultUsb/usbScan.js';
import { initializeVaultOnUsbWithKey } from '../recordVaultUsb/vaultSession.js';
import { isPlaintextVaultKey } from '../recordVaultUsb/vaultCrypto.js';
import {
  vaultMetaUsesPlaintextStorage,
  isRecordVaultIconEncryptionEnabled
} from '../recordVaultIconEncryption.js';
import { rvCloudConnSnapshot, rvCloudDebug, rvCloudError, rvCloudLog, rvCloudWarn } from '../recordVaultCloudDebugLog.js';
import {
...
import { oneDriveStagingMountPath as stagingMountPath } from '../recordVaultOneDriveStagingRoot.js';

const ONEDRIVE_QUEUE_PREFIX = 'v1:record_vault:onedrive:queue:';
const ONEDRIVE_QUEUED_SET_PREFIX = 'v1:record_vault:onedrive:queued_set:';
const ONEDRIVE_INFLIGHT_PREFIX = 'v1:record_vault:onedrive:inflight:';
const ONEDRIVE_INFLIGHT_PATH_PREFIX = 'v1:record_vault:onedrive:inflight_path:';
const ONEDRIVE_PUMP_LOCK_PREFIX = 'v1:record_vault:onedrive:pump_lock:';
const ONEDRIVE_MOUNT_PREFIX = 'v1:record_vault:onedrive:queue_mount:';
...
export async function formatEmptyOneDriveVaultFolderIfNeeded(singlesId, accessToken, folderId, keyOrMaterial = null) {
  ...
  const effectiveKey = isRecordVaultIconEncryptionEnabled() ? key : null;
  if (isRecordVaultIconEncryptionEnabled() && isPlaintextVaultKey(effectiveKey)) {
...
async function findOneDriveVaultRootFolderWithMeta(accessToken) {
  for (const folderName of listOneDriveVaultRootFolderNamesToPurge()) {
...
async function resolveOneDriveVaultFolderId(singlesId, accessToken, conn) {
  ...
  // Keep stored folder when it still holds vault.meta.json (do not abandon for name mismatch).
  if (storedId && (await oneDriveFolderHasVaultMeta(accessToken, storedId))) {
    return storedId;
  }
  ...
  if (storedId) {
    try {
      ...
      if (item?.folder && isOneDriveVaultRootFolderName(item?.name)) {
        return storedId;
      }
      rvCloudWarn('OneDrive', 'stored folderId has no vault meta — searching drive root', {
        storedId,
        name: item?.name || null,
        expected: getOneDriveVaultFolderName()
      });
    } catch (err) {
  ...
  try {
    ...
    if (meta?.vaultId) {
      try {
        ...
        if (hasVault && isRecordVaultIconEncryptionEnabled()) {
    ...
    } else if (metaFile?.id && hasVault && isRecordVaultIconEncryptionEnabled()) {
      needsReformat = true;
    } else if (metaFile?.id && isRecordVaultIconEncryptionEnabled() && !meta?.vaultId) {
      needsReformat = true;
    }
