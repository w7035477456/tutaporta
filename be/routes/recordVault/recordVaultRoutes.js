import {
  buildRecordVaultSearchChain,
  evaluateRecordVaultSearchChain
} from '../../utils/recordVaultSearch.js';
import { formatDefaultRecordVaultNoteTitle } from '../../utils/recordVaultDefaultTitle.js';
...
import {
  canNativeOpenRecordVaultExtension,
  isRecordVaultNativeOpenSupported,
  openBufferInMacNativeApp
} from '../../utils/recordVaultNativeOpen.js';
import {
  extensionFromFileName,
  isAllowedRecordVaultFileExtension,
  mimeTypeForRecordVaultExtension
} from '../../utils/recordVaultFileFormats.js';
...
function parseFileDataUrl(dataUrl, fileName = '') {
  ...
  if (!ext || !isAllowedRecordVaultFileExtension(ext)) {
  ...
  return {
    buffer,
    contentType: contentType || mimeTypeForRecordVaultExtension(ext),
    ext,
...
/** GET /api/recordVault */
export async function getRecordVaultTree(req, res) {
...
/** GET /api/recordVault/notes/:noteId — full note content (lazy load; one note at a time). */
export async function getRecordVaultNote(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** POST /api/recordVault/notebooks */
export async function createRecordVaultNotebook(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  try {
    const name = String(req.body?.notebook_name ?? '').trim() || 'New Notebook';
...
/** PATCH /api/recordVault/notebooks/:notebookId */
export async function updateRecordVaultNotebook(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** DELETE /api/recordVault/notebooks/:notebookId */
export async function deleteRecordVaultNotebook(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** POST /api/recordVault/notebooks/:notebookId/notes */
export async function createRecordVaultNote(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  ...
  try {
    ...
    if (!noteName) {
      ...
      noteName = formatDefaultRecordVaultNoteTitle(nbIdx >= 0 ? nbIdx + 1 : 1, noteCount + 1);
    }
    const parsedImage = parseImageDataUrl(req.body?.image);
...
/** PATCH /api/recordVault/notes/:noteId */
export async function updateRecordVaultNote(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  if (rejectInnerPinSecretsInBody(req, res)) return;
  ...
  try {
    ...
    console.error('[updateRecordVaultNote]', err?.message || err);
...
/** POST /api/recordVault/notes/move-image */
export async function moveRecordVaultNoteImage(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** DELETE /api/recordVault/notes/:noteId */
export async function deleteRecordVaultNote(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** GET /api/recordVault/notes/:noteId/image[/top|bottom] */
export async function getRecordVaultNoteImage(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** GET /api/recordVault/notes/:noteId/extra-images/:imageId */
export async function getRecordVaultNoteExtraImage(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** POST /api/recordVault/notes/:noteId/extra-images */
export async function uploadRecordVaultNoteExtraImage(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** DELETE /api/recordVault/notes/:noteId/extra-images/:imageId */
export async function deleteRecordVaultNoteExtraImage(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  ...
  try {
    ...
    console.error('[deleteRecordVaultNoteExtraImage]', err?.message || err);
...
/** PUT /api/recordVault/notebooks/reorder */
export async function reorderRecordVaultNotebooks(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** PUT /api/recordVault/notebooks/:notebookId/notes/reorder */
export async function reorderRecordVaultNotes(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** POST /api/recordVault/shortcuts */
export async function createRecordVaultShortcut(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** DELETE /api/recordVault/shortcuts/:shortcutId */
export async function deleteRecordVaultShortcut(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** PUT /api/recordVault/shortcuts/reorder */
export async function reorderRecordVaultShortcuts(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** GET /api/recordVault/notes/:noteId/attachments/:attachmentId */
export async function getRecordVaultNoteAttachment(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
...
/** POST /api/recordVault/notes/:noteId/attachments/:attachmentId/open-native — Mac only; opens Word/Excel via `open`. */
export async function openRecordVaultNoteAttachmentNative(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  ...
  try {
    ...
    const ext = String(file.fileName || '')
      ...
      .split('.')
      .pop();
    if (!canNativeOpenRecordVaultExtension(ext)) {
...
/** POST /api/recordVault/notes/:noteId/attachments */
export async function uploadRecordVaultNoteAttachment(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  ...
  if (!parsed) {
    console.error('[uploadRecordVaultNoteAttachment] rejected', {
      noteId,
...
/** DELETE /api/recordVault/notes/:noteId/attachments/:attachmentId */
export async function deleteRecordVaultNoteAttachment(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  ...
  try {
    ...
    console.error('[deleteRecordVaultNoteAttachment]', err?.message || err);
...
/** GET /api/recordVault/search */
export async function searchRecordVaultNotes(req, res) {
  const session = await requireVaultSession(req, res);
  if (!session) return;
  ...
  const chain = buildRecordVaultSearchChain(q1, q2, q3, op1, op2);
  if (!chain.length) {
